#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-07-25.
...
# Actual results from July 20, 2026 (sourced via ESPN, MLB.com, Baseball-Reference)
actual_results = {
...
for c in challengers:
    ...
    days_alive = (date(2026, 7, 25) - date(int(born[:4]), int(born[5:7]), int(born[8:10]))).days
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (July 25, 2026)
# ═══════════════════════════════════════════════════════════════════════
...
# Saturday July 25, 2026 — MLB only (NBA/NHL/NFL off-season)
# 15 games. Odds sourced from FanDuel, BetMGM, DraftKings, Covers, CBS Sports.
# Records/stats estimated from standings data and recent form through July 24.
# BOS on historic 15-2 run since July 1. ATL leads NL at 60-40.

games_raw = [
    # 1. KC Royals @ DET Tigers — 1:10 PM ET
    # KC ~38-58, DET ~51-47. DET heavy home fav. Mize vs Wacha.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Detroit Tigers",
    ...
    # 2. ARI Diamondbacks @ WSH Nationals — 4:05 PM ET
    # ARI ~46-50, WSH ~51-49. WSH home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 3. LAA Angels @ SF Giants — 4:05 PM ET
    # LAA ~38-58, SF ~44-53. SF home fav. Robbie Ray vs Ryan Johnson.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 4. TOR Blue Jays @ BOS Red Sox — 4:10 PM ET
    # TOR ~47-53, BOS ~52-48. Pick'em. BOS on 15-2 tear since Jul 1.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Boston Red Sox",
    ...
    # 5. SD Padres @ MIA Marlins — 4:10 PM ET
    # SD ~46-51, MIA ~48-48. MIA home fav. MIA on 10-game losing streak per search.
    # Wait — the search said "Miami's 10-game skid" so SD might be the smart side.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Miami Marlins",
    ...
    # 6. NYY Yankees @ PHI Phillies — 6:05 PM ET
    # NYY ~52-46, PHI ~55-45. PHI home fav. Marquee series.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 7. CLE Guardians @ TB Rays — 6:10 PM ET
    # CLE ~50-48, TB ~58-42 (1st AL East). TB home fav.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Tampa Bay Rays",
    ...
    # 8. CHC Cubs @ PIT Pirates — 6:40 PM ET
    # CHC ~55-45, PIT ~47-49. PIT home fav (pitching edge).
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 9. ATL Braves @ BAL Orioles — 7:05 PM ET
    # ATL ~60-40 (best in NL), BAL ~50-50. Near pick'em. ATL slight dog on road.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 10. OAK Athletics @ MIN Twins — 7:10 PM ET
    # OAK ~36-60, MIN ~52-48. MIN heavy home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 11. HOU Astros @ CWS White Sox — 7:10 PM ET
    # HOU ~49-52, CWS ~53-47. CWS home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 12. COL Rockies @ MIL Brewers — 7:10 PM ET
    # COL ~38-58, MIL ~58-38. MIL heavy home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 13. LAD Dodgers @ NYM Mets — 7:15 PM ET (FOX)
    # LAD ~62-38 (best in MLB), NYM ~43-55. LAD road fav.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "New York Mets",
    ...
    # 14. CIN Reds @ STL Cardinals — 7:15 PM ET
    # CIN ~47-50, STL ~52-48. STL home fav. Greene vs Pallante.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 15. SEA Mariners @ TEX Rangers — 7:15 PM ET
    # SEA ~48-50, TEX ~50-50. Near pick'em. SEA slight road fav.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Texas Rangers",
...
pred_data = {
    ...
    "metadata": {
        ...
        "data_sources": ["web search (ESPN, MLB.com, FanDuel, DraftKings, BetMGM, Covers, CBS Sports, Baseball-Reference)"],
        "note": "ODDS_API_KEY not configured; odds sourced from web search. Saturday 15-game slate."
    }
}
...
# ── Display results ──────────────────────────────────────────────────
print(f"\n{'=' * 90}")
print(f" MLB — Saturday July 25, 2026")
print(f"{'=' * 90}")
...
metrics["rolling_7d"] = {
    ...
    "ev_realized": round((jul20_bet_wins * 0.909 - (jul20_bet_total - jul20_bet_wins)) / max(jul20_bet_total, 1), 4),
...
metrics["today_summary"] = {
    ...
    "sports": ["baseball_mlb"],
    "notes": f"MLB Saturday slate (15 games). Eval Jul 20: {eval_correct}/{eval_games} correct ({eval_acc:.0%}). Jul 20 bets: {jul20_bet_wins}/{jul20_bet_total} won. CLE 13-4 rout of MIN, BOS extended historic win streak to 14."
}
