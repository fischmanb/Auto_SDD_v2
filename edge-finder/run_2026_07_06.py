#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-07-06.
...
for c in challengers:
    born = c.get("born", "2026-03-24")
    days_alive = (date(2026, 7, 6) - date(int(born[:4]), int(born[5:7]), int(born[8:10]))).days
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (July 6, 2026)
# ═══════════════════════════════════════════════════════════════════════
...
# MLB only — 8-game Monday slate. New series starting for most teams.

games_raw = [
    # 1. PHI Phillies (50-40) @ KC Royals (36-54) — 2:10 PM ET
    # Cristopher Sanchez (10-3, 2.00 ERA) vs Noah Cameron (4-6, 4.95 ERA).
    # PHI big road favorite. KC worst record in AL Central.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 2. NYY Yankees (49-40) @ TB Rays (52-35) — 6:40 PM ET
    # Cam Schlittler vs Griffin Jax. New series. Rays slight home fav.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Tampa Bay Rays",
    ...
    # 3. HOU Astros (45-47) @ WAS Nationals (46-45) — 6:45 PM ET
    # Mike Burrows vs Miles Mikolas. New series. WAS slight home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 4. NYM Mets (37-53) @ ATL Braves (52-36) — 7:15 PM ET
    # Freddy Peralta vs Reynaldo Lopez. ATL home fav, NYM struggling.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Atlanta Braves",
    ...
    # 5. MIL Brewers (55-33) @ STL Cardinals (47-40) — 7:45 PM ET
    # Shane Drohan vs Dustin May. New 5-game series. Near pick'em.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 6. ARI Diamondbacks @ SD Padres — 9:40 PM ET
    # New series at Petco Park. SD slight home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 7. TOR Blue Jays @ SF Giants — 9:45 PM ET
    # New series. SF slight home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 8. COL Rockies (37-54) @ LAD Dodgers (59-32) — 10:10 PM ET
    # Eric Lauer vs Kyle Freeland. LAD big home fav. Best record in MLB.
    {
        "sport": "baseball_mlb",
        "home": {
...
# ── Display results ─────────────────────────────────────────────────
print(f"\n{'=' * 90}")
print(f" MLB — Monday July 6, 2026")
print(f"{'=' * 90}")
...
metrics["today_summary"] = {
    ...
    "notes": f"Monday 8-game slate. {total_bets_today} BETs, {total_leans_today} LEANs. Eval Jul 5: {eval_correct}/{eval_games} correct."
}

metrics["champion_history"] = metrics.get("champion_history", [])
if promotion_msg:
    metrics["champion_history"].append({
        "id": champion["id"],
        "promoted_on": TODAY,
        "reason": promotion_msg
    })
