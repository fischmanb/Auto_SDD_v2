#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-07-22.
Phase 1: Eval July 20 predictions (13 MLB games played, 2 postponed).
Phase 2-5: Simulate tonight's 15 MLB matchups (17 total games incl. doubleheaders).
"""
...
# PIT @ NYY and BAL @ BOS postponed (rain) — rescheduled as July 22 doubleheaders.
actual_results = {
    "Minnesota Twins @ Cleveland Guardians": {"winner": "home", "home_score": 5, "away_score": 2},
    "Tampa Bay Rays @ Toronto Blue Jays": {"winner": "away", "home_score": 2, "away_score": 12},
    "Los Angeles Dodgers @ Philadelphia Phillies": {"winner": "away", "home_score": 1, "away_score": 2},
    "San Diego Padres @ Atlanta Braves": {"winner": "away", "home_score": 3, "away_score": 8},
    "San Francisco Giants @ Kansas City Royals": {"winner": "home", "home_score": 3, "away_score": 2},
    "New York Mets @ Milwaukee Brewers": {"winner": "away", "home_score": 0, "away_score": 4},
    ...
    "Washington Nationals @ Colorado Rockies": {"winner": "away", "home_score": 6, "away_score": 9},
...
print(f"\nEvaluated {total_games} games from {EVAL_DATE} (2 postponed)")
...
for c in challengers:
    ...
    days_alive = (date(2026, 7, 22) - date(int(born[:4]), int(born[5:7]), int(born[8:10]))).days
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (July 22, 2026)
# ═══════════════════════════════════════════════════════════════════════
...
# Wednesday July 22, 2026 — MLB only (NBA/NHL/NFL off-season)
# 15 matchups (17 total games: BAL@BOS and PIT@NYY doubleheaders from Jul 20 PPD).
# Odds sourced from BetMGM, DraftKings, FanDuel, Covers, ESPN, NBC Sports.
# Records approximate through July 21. Stats estimated from standings and recent form.

games_raw = [
    # 1. PIT Pirates @ NYY Yankees — DH G1 1:35 PM ET (makeup from Jul 20 PPD)
    # PIT 52-49, NYY 56-44. NYY heavy home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 2. SF Giants @ KC Royals — 2:10 PM ET
    # SF 42-59, KC 42-60. SF road fav. Battle of cellar dwellers.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 3. NYM Mets @ MIL Brewers — 2:10 PM ET
    # NYM 43-59, MIL 63-38. MIL home fav. Best record in NL.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 4. WSH Nationals @ COL Rockies — 3:10 PM ET (Coors Field)
    # WSH 51-51, COL ~38-62. WSH road fav. Coors inflates total to 11.5.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 5. OAK Athletics @ ARI Diamondbacks — 3:40 PM ET
    # OAK 43-58, ARI ~52-48. ARI home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 6. CIN Reds @ SEA Mariners — 3:40 PM ET
    # CIN ~47-53, SEA 51-51. SEA home fav. SEA off 8-0 shutout of CIN.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 7. STL Cardinals @ LAA Angels — 4:07 PM ET
    # STL ~51-48, LAA 41-61. STL slight road fav. LAA worst in AL West.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 8. MIN Twins @ CLE Guardians — 6:40 PM ET
    # MIN 49-53, CLE 54-48. CLE home fav. CLE won July 20 game 5-2.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 9. LAD Dodgers @ PHI Phillies — 6:40 PM ET
    # LAD 62-36 (best in MLB), PHI 56-46. LAD road fav. Marquee series.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 10. TB Rays @ TOR Blue Jays — 7:07 PM ET
    # TB 58-42 (1st AL East), TOR 46-55. Near pick'em (TOR slight home fav).
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 11. BAL Orioles @ BOS Red Sox — DH G1 1:35 PM ET (makeup from Jul 20 PPD)
    # BAL 49-52, BOS 51-48. BOS home fav at Fenway.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Boston Red Sox",
    ...
    # 12. SD Padres @ ATL Braves — 7:15 PM ET
    # SD ~56-44, ATL 58-42. ATL home fav. ATL best in NL East.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Atlanta Braves",
    ...
    # 13. CWS White Sox @ TEX Rangers — 8:05 PM ET
    # CWS 53-47, TEX 51-50. Pick'em. CWS off 10-3 rout of TEX on Jul 20.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Texas Rangers",
    ...
    # 14. DET Tigers @ CHC Cubs — 8:10 PM ET
    # DET 47-54, CHC 57-44. CHC slight home fav. DET won Jul 20 game 8-6.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Chicago Cubs",
    ...
    # 15. MIA Marlins @ HOU Astros — 8:10 PM ET
    # MIA 52-50, HOU 49-54. HOU home fav. HOU won Jul 20 game 8-5.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Houston Astros",
...
pred_data = {
    ...
    "metadata": {
        ...
        "data_sources": ["web search (ESPN, MLB.com, FanDuel, DraftKings, BetMGM, Covers, NBC Sports)"],
        "note": "ODDS_API_KEY not configured; odds sourced from web search. Wed slate incl. BAL@BOS and PIT@NYY doubleheaders (Jul 20 PPD makeups). 15 matchups, 17 total games."
    }
}
...
# ── Display results ──────────────────────────────────────────────────
print(f"\n{'=' * 90}")
print(f" MLB — Wednesday July 22, 2026")
print(f"{'=' * 90}")
...
jul20_bets = [p for p in past_preds["predictions"] if p["verdict"] == "BET"]
jul20_bet_wins = 0
jul20_bet_total = len(jul20_bets)
for bp in jul20_bets:
    gk = bp["game"]
...
new_all_bets = prev_all["total_bets_recommended"] + jul20_bet_total
new_all_bets_won = prev_all["total_bets_won"] + jul20_bet_wins

metrics["last_updated"] = TODAY
metrics["rolling_7d"] = {
    "accuracy": round(eval_acc, 4),
    "ev_realized": round(jul20_bet_wins / jul20_bet_total if jul20_bet_total > 0 else 0, 4),
    "total_games": eval_games,
    "total_correct": eval_correct
}
metrics["rolling_30d"] = {
    "accuracy": round(new_all_correct / new_all_games if new_all_games > 0 else 0, 4),
    "ev_realized": round(new_all_bets_won / new_all_bets if new_all_bets > 0 else 0, 4),
...
for model in all_models:
    ...
    metrics["variant_performance"][mid] = {
        ...
        "weight": 1.0 if mid == champ_id else next((c["weight"] for c in challengers if c["id"] == mid), 0),
...
metrics["today_summary"] = {
    ...
    "sports": ["baseball_mlb"],
    "notes": f"MLB Wednesday slate (15 matchups, 17 games incl. DH). Eval Jul 20: {eval_correct}/{eval_games} correct ({eval_acc:.0%}). Jul 20 bets: {jul20_bet_wins}/{jul20_bet_total} won. 2 games PPD (PIT@NYY, BAL@BOS) — made up today as DH."
}
...
print(f"7-day:  {eval_acc:.0%} accuracy | {eval_correct}/{eval_games} games")
print(f"All-time: {new_all_correct}/{new_all_games} = {new_all_correct/new_all_games:.1%} accuracy")
print(f"All-time bets: {new_all_bets_won}/{new_all_bets} won")
print()
cw = ", ".join(f"{c['id'].replace('-v1','')}={c['weight']}" for c in challengers)
print(f"Challenger weights: {cw}")
if assumptions.get("graveyard"):
    grav = ", ".join(f"{g['id']} (died {g['died']}, {g['lifetime_accuracy']:.0%})" for g in assumptions["graveyard"])
    print(f"Graveyard: {grav}")

print(f"\nToday: {len(bets)} BETs, {len(leans)} LEANs out of {len(predictions)} games")
