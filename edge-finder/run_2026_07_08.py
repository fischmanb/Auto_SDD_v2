#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-07-08.
...
# Actual scores from July 7, 2026 — sourced via ESPN/MLB.com/CBS Sports
actual_results = {
    "Milwaukee Brewers @ St. Louis Cardinals": {"winner": "away", "home_score": 3, "away_score": 4},
...
for c in challengers:
    ...
    days_alive = (date(2026, 7, 8) - date(int(born[:4]), int(born[5:7]), int(born[8:10]))).days
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (July 8, 2026)
# ═══════════════════════════════════════════════════════════════════════
...
# Wednesday July 8, 2026 — MLB only (NBA/NHL/NFL off-season)
# Odds sourced from FanDuel/DraftKings/BetMGM/Covers/ActionNetwork

games_raw = [
    # 1. TOR Blue Jays @ SF Giants
    # Cease vs Webb. TOR -122, SF -102.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # Melton vs Springs. ATH +123, DET -126.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Detroit Tigers",
    ...
    # Holmes vs Jones. ATL +102, PIT -120.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 5. NYY Yankees @ TB Rays
    # Cole vs McClanahan. NYY +108, TB -126.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Tampa Bay Rays",
    ...
    # 6. HOU Astros @ WAS Nationals
    # Arrighetti vs Griffin. HOU +116, WAS -136.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 7. MIL Brewers @ STL Cardinals
    # Harrison (8-1) vs McGreevy (3-7). MIL -138, STL +118.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # Bennett (3-3, 3.10) vs Martin (9-3, 3.08). BOS -105, CWS -115.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # Cecconi vs Prielipp. ARI +120, SD -142.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 12. SEA Mariners @ MIA Marlins
    # Kirby vs ??? . SEA -138, MIA +118.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Miami Marlins",
...
# ── Display results ──────────────────────────────────────────────────
print(f"\n{'=' * 90}")
print(f" MLB — Wednesday July 8, 2026")
print(f"{'=' * 90}")
...
metrics["today_summary"] = {
    ...
    "notes": f"MLB Wednesday slate (13 games). {total_bets_today} BETs, {total_leans_today} LEANs. Eval Jul 7: {eval_correct}/{eval_games} correct ({eval_acc:.1%})."
}
...
print(f"  Jul 7 eval: {eval_correct}/{eval_games} = {eval_acc:.1%} accuracy")
print(f"  Jul 7 bets: {jul7_bet_wins}/{jul7_bet_total} = {jul7_bet_wins/max(jul7_bet_total,1):.1%} hit rate")
