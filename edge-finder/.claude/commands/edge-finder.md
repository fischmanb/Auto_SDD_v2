...
**Working directory**: The `edge-finder/` repo root. All file paths are relative to this repo.
...
### Step 1.7: Commit
```bash
cd /home/user/edge-finder && git add assumptions.json results.tsv metrics.json && git commit -m "eval YYYY-MM-DD: accuracy X%, champion: {id}, [changes]"
```
...
```bash
cd /home/user/edge-finder && python sim.py --batch /tmp/edge_finder_batch.json
```
...
```bash
cd /home/user/edge-finder && git add predictions/ metrics.json && git commit -m "predict YYYY-MM-DD: N games, M bets recommended"
```
