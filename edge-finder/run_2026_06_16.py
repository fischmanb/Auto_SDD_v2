#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-06-16 (Tuesday).
Phase 1: Eval June 15 predictions, update weights.
...
# ════════════════════════════════════════════════════════════════════════
# PHASE 1 — EVALUATE JUNE 15 PREDICTIONS
# ════════════════════════════════════════════════════════════════════════

print("=" * 78)
print(" PHASE 1 — Evaluating 2026-06-15 predictions")
print("=" * 78)
...
# Actual results fetched via web search (June 15, 2026)
actual_results = {
    "Miami Marlins @ Philadelphia Phillies": {"winner": "home", "home_score": 7, "away_score": 0},
    "Kansas City Royals @ Washington Nationals": {"winner": "home", "home_score": 7, "away_score": 3},
    "New York Mets @ Cincinnati Reds": {"winner": "home", "home_score": 9, "away_score": 0},
    "San Diego Padres @ St. Louis Cardinals": {"winner": "home", "home_score": 3, "away_score": 0},
    "Colorado Rockies @ Chicago Cubs": {"winner": "home", "home_score": 5, "away_score": 4},
    "Minnesota Twins @ Texas Rangers": {"winner": "away", "home_score": 2, "away_score": 4},
    "Detroit Tigers @ Houston Astros": {"winner": "away", "home_score": 3, "away_score": 9},
    "Los Angeles Angels @ Arizona Diamondbacks": {"winner": "home", "home_score": 4, "away_score": 3},
    "Pittsburgh Pirates @ Oakland Athletics": {"winner": "home", "home_score": 11, "away_score": 2},
    "Tampa Bay Rays @ Los Angeles Dodgers": {"winner": "home", "home_score": 4, "away_score": 3},
}
...
# Rolling 10-day accuracy (June 6-15)
print("\n  Rolling 10-day accuracy (June 6-15):")
...
with open(os.path.join(BASE, "results.tsv")) as f:
    for line in f:
        ...
        if len(parts) < 11:
            continue
        d = parts[0]
        if d < "2026-06-06" or d > "2026-06-15":
            continue
        mid = parts[3]
        hit_val = int(parts[9])
...
for challenger in assumptions["challengers"]:
    cid = challenger["id"]
    if challenger["weight"] <= 0.15 and challenger["born"] < "2026-06-11":
...
# ════════════════════════════════════════════════════════════════════════
# PHASE 2 — DATA COLLECTION (Tonight's Games: June 16, 2026)
# ════════════════════════════════════════════════════════════════════════
# NHL: Season over
# NBA: Off-season
# NFL: Off-season
# MLB: Regular season — Tuesday schedule, 13 games
#
# Odds via web search (FanDuel primary, cross-checked DraftKings/BetMGM).
# Stats carried forward from June 15 run for returning matchups;
# estimated from records/standings for new matchups.
...
print(f"  {len(games_tonight)} MLB games tonight (Tuesday)")
...
print("\n")
print("+" + "-" * 85 + "+")
print(f"| MLB -- Tuesday Jun 16, 2026{' ' * 57}|")
...
metrics.update({
    ...
    "today_summary": {
        ...
        "leans": lean_count,
        "sports": ["baseball_mlb"],
        "notes": f"MLB only (NHL over, NBA/NFL off-season). {bet_count} BETs, {lean_count} LEANs. Eval Jun 15: {champion_correct_count}/{champion_total_count} correct (8/10 home wins, home teams dominated)."
    },
})
