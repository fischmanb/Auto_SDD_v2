#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-07-03.
Phase 1: Eval July 2 predictions, update weights.
...
# ════════════════════════════════════════════════════════════════════════
# PHASE 1 — EVALUATE JULY 2 PREDICTIONS
# ════════════════════════════════════════════════════════════════════════

print("=" * 78)
print(" PHASE 1 — Evaluating 2026-07-02 predictions")
print("=" * 78)
...
actual_results = {
    "Chicago White Sox @ Cleveland Guardians": {"winner": "home", "home_score": 6, "away_score": 5},
    "St. Louis Cardinals @ Atlanta Braves": {"winner": "away", "home_score": 5, "away_score": 11},
    "Tampa Bay Rays @ Kansas City Royals": {"winner": "away", "home_score": 2, "away_score": 5},
    "Detroit Tigers @ Texas Rangers": {"winner": "home", "home_score": 10, "away_score": 4},
    "Los Angeles Angels @ Seattle Mariners": {"winner": "home", "home_score": 1, "away_score": 0},
    "San Diego Padres @ Los Angeles Dodgers": {"winner": "home", "home_score": 12, "away_score": 7},
}
...
for c in challengers:
    born = c.get("born", "2026-03-24")
    days_alive = (date(2026, 7, 3) - date(int(born[:4]), int(born[5:7]), int(born[8:10]))).days
...
# ════════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (July 3, 2026)
# ════════════════════════════════════════════════════════════════════════
...
# MLB only — NBA and NHL seasons are over.
# 13-game Friday slate, July 3, 2026.

games_raw = [
    # 1. St. Louis Cardinals @ Chicago Cubs (4:05 PM ET)
    # Cubs -126 / Cardinals +105. O/U 8.5.
    # Cubs home fav. Both teams around .500.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Chicago Cubs",
    ...
    # 2. Pittsburgh Pirates @ Washington Nationals (6:45 PM ET)
    # Nationals -136 / Pirates +114. O/U 7.5.
    # Keller (PIT) pitching. Nationals home favorite.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 3. Minnesota Twins @ New York Yankees (7:05 PM ET)
    # Yankees -188 / Twins +158. O/U 8.5.
    # Gerrit Cole on the mound for NYY. Yankees 2nd biggest fav on slate.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 4. Baltimore Orioles @ Cincinnati Reds (7:10 PM ET)
    # Orioles -120 / Reds +100. O/U 8.0.
    # Trevor Rogers dominant (1.40 ERA last 3 starts) for BAL.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 5. Chicago White Sox @ Cleveland Guardians (7:10 PM ET)
    # Guardians -137 / White Sox +115. O/U 8.5.
    # Game 2 of series. CLE won G1 6-5 walk-off HR.
    # Anthony Kay (CHW, 3.65 xERA last 3) vs CLE starter.
    # CHW 45-40, CLE 45-42. Both atop AL Central.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 6. New York Mets @ Atlanta Braves (7:15 PM ET)
    # Braves -116 / Mets +100. O/U 8.5.
    # Braves 50-34 (2nd best NL). Mets struggling ~37-48.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Atlanta Braves",
    ...
    # 7. San Francisco Giants @ Colorado Rockies (8:10 PM ET)
    # Giants -156 / Rockies +132. O/U 11.0.
    # Coors Field inflates totals. Logan Webb pitching for SF.
    # Ryan Feltner (2.70 ERA last 3) for COL.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 8. Tampa Bay Rays @ Houston Astros (8:15 PM ET)
    # Pick'em: Rays -110 / Astros -110. O/U 8.5.
    # TB best record in AL (50-33). HOU 44-40.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Houston Astros",
    ...
    # 9. Boston Red Sox @ Los Angeles Angels (9:38 PM ET)
    # Angels -115 / Red Sox -105. O/U 7.5.
    # Sharp action on under (dropped from 8 to 7.5).
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 10. Miami Marlins @ Sacramento Athletics (9:40 PM ET)
    # Athletics -126 / Marlins +108. O/U 10.5.
    # Tyler Phillips pitching for SAC.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 11. Milwaukee Brewers @ Arizona Diamondbacks (9:45 PM ET)
    # Brewers -155 / D-Backs +130. O/U 9.0.
    # MIL 52-31 (2nd best MLB). ARI ~42-42.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 12. San Diego Padres @ Los Angeles Dodgers (10:10 PM ET)
    # Dodgers -250 / Padres +205. O/U 8.0.
    # LAD biggest fav on slate. 56-31, best record in MLB.
    # Mookie Betts returned to lineup.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 13. Toronto Blue Jays @ Seattle Mariners (10:10 PM ET)
    # Blue Jays -131 / Mariners +101. O/U 7.5.
    # SEA 45-43 after sweeping LAA. TOR ~41-43.
    {
        "sport": "baseball_mlb",
        "home": {
...
# ── Display results ─────────────────────────────────────────────────
print(f"\n{'=' * 90}")
print(f" MLB — Friday July 3, 2026")
print(f"{'=' * 90}")
...
jul2_bets = [p for p in past_preds["predictions"] if p["verdict"] == "BET"]
jul2_bet_wins = 0
jul2_bet_total = len(jul2_bets)
for bp in jul2_bets:
    gk = bp["game"]
...
new_all_bets = prev_all["total_bets_recommended"] + jul2_bet_total
new_all_bets_won = prev_all["total_bets_won"] + jul2_bet_wins

metrics["last_updated"] = TODAY
metrics["rolling_7d"] = {
    "accuracy": round(eval_correct / eval_games, 4) if eval_games > 0 else 0.5,
    "ev_realized": round((jul2_bet_wins * 0.909 - (jul2_bet_total - jul2_bet_wins)) / max(jul2_bet_total, 1), 4),
...
metrics["today_summary"] = {
    ...
    "sports": ["baseball_mlb"],
    "notes": f"MLB only ({len(predictions)} games). {total_bets_today} BETs, {total_leans_today} LEANs. Eval Jul 2: {eval_correct}/{eval_games} correct."
}
...
print(f"  Jul 2 eval: {eval_correct}/{eval_games} = {eval_acc:.1%} accuracy")
print(f"  Jul 2 bets: {jul2_bet_wins}/{jul2_bet_total} = {jul2_bet_wins/max(jul2_bet_total,1):.1%} hit rate")
