#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-07-27.
Phase 1: Eval July 26 predictions (15 MLB games).
Phase 2-5: Simulate tonight's 12 MLB games (Monday slate, new series), produce blended predictions.
"""
...
TODAY = "2026-07-27"
EVAL_DATE = "2026-07-26"

# ════════════════════════════════════════════════════════════════════════
# PHASE 1 — EVALUATE JULY 26 PREDICTIONS
# ════════════════════════════════════════════════════════════════════════

print("=" * 78)
print(" PHASE 1 — Evaluating 2026-07-26 predictions")
print("=" * 78)
...
# Actual results from July 26, 2026 (sourced via web search: MLB.com Gameday, ESPN, Baseball-Reference)
actual_results = {
    "Kansas City Royals @ Detroit Tigers": {"winner": "away", "home_score": 4, "away_score": 5},
    "Arizona Diamondbacks @ Washington Nationals": {"winner": "home", "home_score": 10, "away_score": 7},
    "Los Angeles Angels @ San Francisco Giants": {"winner": "away", "home_score": 3, "away_score": 4},
    "Toronto Blue Jays @ Boston Red Sox": {"winner": "home", "home_score": 6, "away_score": 1},
    "San Diego Padres @ Miami Marlins": {"winner": "away", "home_score": 3, "away_score": 5},
    "New York Yankees @ Philadelphia Phillies": {"winner": "home", "home_score": 11, "away_score": 4},
    "Cleveland Guardians @ Tampa Bay Rays": {"winner": "home", "home_score": 1, "away_score": 0},
    "Chicago Cubs @ Pittsburgh Pirates": {"winner": "home", "home_score": 8, "away_score": 7},
    "Atlanta Braves @ Baltimore Orioles": {"winner": "away", "home_score": 2, "away_score": 3},
    "Oakland Athletics @ Minnesota Twins": {"winner": "home", "home_score": 11, "away_score": 8},
    "Houston Astros @ Chicago White Sox": {"winner": "home", "home_score": 12, "away_score": 3},
    "Colorado Rockies @ Milwaukee Brewers": {"winner": "home", "home_score": 11, "away_score": 2},
    "Los Angeles Dodgers @ New York Mets": {"winner": "home", "home_score": 8, "away_score": 3},
    "Cincinnati Reds @ St. Louis Cardinals": {"winner": "away", "home_score": 3, "away_score": 5},
    "Seattle Mariners @ Texas Rangers": {"winner": "away", "home_score": 4, "away_score": 6},
}
...
for c in challengers:
    ...
    days_alive = (date(2026, 7, 27) - date(int(born[:4]), int(born[5:7]), int(born[8:10]))).days
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (July 27, 2026)
# ═══════════════════════════════════════════════════════════════════════
...
# Monday July 27, 2026 — MLB only (NBA/NHL/NFL off-season)
# 12 games (mostly new series openers). Odds sourced from FanDuel, DraftKings,
# BetMGM, NBC Sports, SportsGrid, Covers, SI.
# MIA losing streak at 12. CWS blew out HOU 12-3 Sun but remain worst in AL.
# PHI dominant at home. BOS strong road favorite at OAK.
# SEA @ TEX is a continued series (4-game set).

games_raw = [
    # 1. SEA Mariners @ TEX Rangers — Monday 2:35 PM ET (series finale)
    # SEA road fav. SEA -139 / TEX +114. SEA won last night 6-4.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Texas Rangers",
    ...
    # 2. BAL Orioles @ DET Tigers — Monday 6:40 PM ET (new series)
    # DET slim home fav. DET -120 / BAL -101 (DK). Bradish vs Montero.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Detroit Tigers",
    ...
    # 3. ARI Diamondbacks @ PIT Pirates — Monday 6:40 PM ET (new series)
    # PIT slight home fav. PIT -124 / ARI +106. Keller(4.90) vs Kelly(5.12).
    # Total at 9.0 — both pitchers struggling. Near pick'em.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 4. PHI Phillies @ MIA Marlins — Monday 6:40 PM ET (new series)
    # PHI heavy road fav. PHI -164 / MIA +138. Wheeler (sub-2.40 ERA) vs Phillips (3.28).
    # MIA on franchise-record 12-game losing streak.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Miami Marlins",
    ...
    # 5. TOR Blue Jays @ WSH Nationals — Monday 6:45 PM ET (new series)
    # WSH home fav. WSH -135 / TOR +110. Alvarez vs Scherzer.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 6. CLE Guardians @ CIN Reds — Monday 7:10 PM ET (new series)
    # CIN heavy home fav. CIN -174 / CLE +146. Chase Burns(ace) vs Cecconi(5.48 xERA).
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 7. ATL Braves @ NYM Mets — Monday 7:10 PM ET (new series)
    # ATL slight road fav. ATL -116 / NYM -100. ATL lead NL East.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "New York Mets",
    ...
    # 8. NYY Yankees @ CWS White Sox — Monday 7:40 PM ET (new series)
    # NYY heavy road fav. NYY -156 / CWS +129. CWS worst in AL.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 9. CHC Cubs @ STL Cardinals — Monday 7:45 PM ET (new series)
    # CHC slight road fav. CHC -120 / STL +102.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 10. HOU Astros @ LAA Angels — Monday 9:38 PM ET (new series)
    # HOU slight road fav. HOU -121 / LAA -101. Near pick'em.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 11. BOS Red Sox @ OAK Athletics — Monday 9:40 PM ET (new series)
    # BOS heavy road fav. BOS -185 / OAK +152.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 12. MIL Brewers @ SF Giants — Monday 9:45 PM ET (new series)
    # MIL road fav. MIL -134 / SF +114. Sproat(5.01) vs Mahle(5.31).
    {
        "sport": "baseball_mlb",
        "home": {
...
output = {
    ...
    "metadata": {
        ...
        "data_sources": [
            "web search (MLB.com, ESPN, FanDuel, DraftKings, BetMGM, NBC Sports, Covers, SportsGrid, SI)"
        ],
        "note": "ODDS_API_KEY not configured; odds sourced from web search. Monday 12-game slate (new series openers + SEA/TEX finale)."
    }
}
...
metrics["today_summary"] = {
    ...
    "sports": ["baseball_mlb"],
    "notes": f"MLB Monday slate (12 games, new series). Eval Jul 26: {eval_correct}/{eval_total} correct ({eval_correct/eval_total*100:.0f}%). MIA losing streak at 12. CWS stunned HOU 12-3."
}
...
# ── Display results ──────────────────────────────────────────────────
print(f"\n{'=' * 78}")
print(f" RESULTS — MLB Monday Jul 27, 2026")
print(f"{'=' * 78}")
