#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-06-19 (Friday).
Phase 1: Eval June 18 predictions, update weights.
...
# ════════════════════════════════════════════════════════════════════════
# PHASE 1 — EVALUATE JUNE 18 PREDICTIONS
# ════════════════════════════════════════════════════════════════════════

print("=" * 78)
print(" PHASE 1 — Evaluating 2026-06-18 predictions")
print("=" * 78)
...
# Actual results fetched via web search (June 18, 2026)
# Sources: ESPN game recap pages, CBS Sports box scores
actual_results = {
    "Toronto Blue Jays @ Boston Red Sox": {"winner": "away", "home_score": 3, "away_score": 4},
    "Cleveland Guardians @ Milwaukee Brewers": {"winner": "away", "home_score": 2, "away_score": 4},
    "Minnesota Twins @ Texas Rangers": {"winner": "away", "home_score": 3, "away_score": 9},
    "Baltimore Orioles @ Seattle Mariners": {"winner": "home", "home_score": 3, "away_score": 0},
    "New York Mets @ Philadelphia Phillies": {"winner": "away", "home_score": 4, "away_score": 6},
    "Chicago White Sox @ New York Yankees": {"winner": "away", "home_score": 1, "away_score": 5},
    "San Francisco Giants @ Atlanta Braves": {"winner": "postponed", "home_score": 0, "away_score": 0},
    "St. Louis Cardinals @ Kansas City Royals": {"winner": "home", "home_score": 14, "away_score": 6},
    "Oakland Athletics @ Los Angeles Angels": {"winner": "away", "home_score": 0, "away_score": 5},
}
...
print(f"\n  Evaluated {total_games_evaluated} games (1 postponed)")
print(f"  Appended {len(results_lines)} result rows to results.tsv")

# Print model accuracy
print("\n  Model accuracy on June 18 predictions:")
...
for challenger in assumptions["challengers"]:
    cid = challenger["id"]
    if challenger["weight"] <= 0.15 and challenger["born"] < "2026-06-14":
...
# ════════════════════════════════════════════════════════════════════════
# PHASE 2 — DATA COLLECTION (Tonight's Games: June 19, 2026)
# ════════════════════════════════════════════════════════════════════════
# NHL: Season over
# NBA: Off-season
# NFL: Off-season
# MLB: Regular season — Friday schedule, 13 games
#
# Odds via web search (FanDuel primary, cross-checked DraftKings/ESPN).
# Stats carried forward from June 18 run with adjustments for games
# played June 18. Pitching matchups sourced from search results.

print("\n" + "=" * 78)
print(f" PHASE 2 — Tonight's Games & Odds ({TODAY})")
print("=" * 78)

# Team stats (season through June 18, updated from June 18 run + results)
# Format: name, season_ppg, season_opp_ppg, last10_ppg, last10_opp_ppg,
#          season_pace, home_record_pct, away_record_pct, is_b2b, key_injuries
...
# Reload assumptions (may have been updated in Phase 1)
with open(os.path.join(BASE, "assumptions.json")) as f:
    assumptions = json.load(f)
...
for game_key, gdata in game_results.items():
    ...
    # Best EV across bet types (blended)
...
print("\n")
print("┌─────────────────────────────────────────────────────────────────────────────────┐")
print(f"│ MLB — Friday Jun 19, 2026                                                      │")
print("├──────────────────────────────┬────────┬────────┬────────┬───────┬──────┬─────────┤")
...
all_time_games = champ["lifetime_games"]
all_time_correct = champ["lifetime_correct"]
all_time_acc = all_time_correct / all_time_games if all_time_games > 0 else 0

# Update metrics.json
with open(os.path.join(BASE, "metrics.json")) as f:
    metrics = json.load(f)

# Update all-time with today's eval results
...
# Rolling 7-day (approximate: last eval + this eval)
...
metrics["today_summary"] = {
    ...
    "leans": lean_count,
    "sports": ["baseball_mlb"],
    "notes": f"MLB only. {bet_count} BETs, {lean_count} LEANs. Eval Jun 18: {champion_correct_count}/{champion_total_count} correct.",
}
