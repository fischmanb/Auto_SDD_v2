#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-07-20.
Phase 1: Eval July 18 predictions (15 MLB games).
...
actual_results = {
    ...
    "New York Mets @ Philadelphia Phillies": {"winner": "away", "home_score": 1, "away_score": 6},
    ...
    "Texas Rangers @ Atlanta Braves": {"winner": "home", "home_score": 8, "away_score": 5},
    ...
    "Los Angeles Dodgers @ New York Yankees": {"winner": "away", "home_score": 2, "away_score": 8},
...
for c in challengers:
    ...
    days_alive = (date(2026, 7, 20) - date(int(born[:4]), int(born[5:7]), int(born[8:10]))).days
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (July 20, 2026)
# ═══════════════════════════════════════════════════════════════════════
...
# Monday July 20, 2026 — MLB only (NBA/NHL/NFL off-season)
# 15 games, all new series openers.
# Odds sourced from BetMGM, DraftKings, FanDuel, Covers, CBS Sports, ESPN.
# Records approximate through July 19. Stats estimated from standings and recent form.

games_raw = [
    # 1. MIN Twins @ CLE Guardians — 6:40 PM ET
    # MIN ~50-50, CLE ~48-43. MIN road favorite. Series opener.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 2. PIT Pirates @ NYY Yankees — 7:05 PM ET
    # PIT ~45-46, NYY ~49-41. Near pick'em. PIT slight road fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 3. TB Rays @ TOR Blue Jays — 7:07 PM ET
    # TB 57-40 (1st AL East), TOR 46-52. TOR home favorite (Cease pitching).
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 4. BAL Orioles @ BOS Red Sox — 7:10 PM ET
    # BAL ~49-52, BOS ~47-50. BOS home favorite at Fenway.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Boston Red Sox",
    ...
    # 5. LAD Dodgers @ PHI Phillies — 7:10 PM ET
    # LAD 63-37 (best in MLB), PHI 55-45. PHI home fav. Marquee matchup.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 6. SD Padres @ ATL Braves — 7:15 PM ET
    # SD ~44-45, ATL ~57-41. ATL home favorite. SD coming off 19-2 rout of KC.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Atlanta Braves",
    ...
    # 7. SF Giants @ KC Royals — 7:40 PM ET
    # SF ~41-51, KC ~37-54. KC slight home fav. Total 10.5.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 8. NYM Mets @ MIL Brewers — 7:40 PM ET
    # NYM ~42-58, MIL 56-33. MIL heavy fav. Misiorowski pitching.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 9. DET Tigers @ CHC Cubs — 8:05 PM ET
    # DET ~40-51, CHC ~55-44. CHC slight home fav.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Chicago Cubs",
    ...
    # 10. CWS White Sox @ TEX Rangers — 8:05 PM ET
    # CWS ~52-46, TEX ~50-49. TEX heavy home fav (deGrom pitching).
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Texas Rangers",
    ...
    # 11. MIA Marlins @ HOU Astros — 8:10 PM ET
    # MIA ~48-44, HOU ~48-53. HOU home fav.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Houston Astros",
    ...
    # 12. WSH Nationals @ COL Rockies — 8:40 PM ET (Coors Field)
    # WSH ~39-56, COL ~37-55. WSH road fav. Coors inflates total.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 13. OAK Athletics @ ARI Diamondbacks — 9:40 PM ET
    # OAK ~35-57, ARI ~44-46. ARI home fav. OAK coming off 15-1 win.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 14. CIN Reds @ SEA Mariners — 9:40 PM ET
    # CIN ~42-48, SEA ~45-47. SEA home fav.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 15. STL Cardinals @ LAA Angels — 10:10 PM ET
    # STL ~48-40, LAA ~37-55. LAA home fav (Soriano pitching, 8-6 3.49 ERA).
    {
        "sport": "baseball_mlb",
        "home": {
...
pred_data = {
    ...
    "metadata": {
        ...
        "data_sources": ["web search (ESPN, MLB.com, FanDuel, DraftKings, BetMGM, Covers, CBS Sports)"],
        "note": "ODDS_API_KEY not configured; odds sourced from web search. Monday series openers across the board."
    }
}
...
# ── Display results ──────────────────────────────────────────────────
print(f"\n{'=' * 90}")
print(f" MLB — Monday July 20, 2026")
print(f"{'=' * 90}")
...
jul18_bets = [p for p in past_preds["predictions"] if p["verdict"] == "BET"]
jul18_bet_wins = 0
jul18_bet_total = len(jul18_bets)
for bp in jul18_bets:
    gk = bp["game"]
...
new_all_bets = prev_all["total_bets_recommended"] + jul18_bet_total
new_all_bets_won = prev_all["total_bets_won"] + jul18_bet_wins

metrics["last_updated"] = TODAY
metrics["rolling_7d"] = {
    "accuracy": round(eval_correct / eval_games, 4) if eval_games > 0 else 0.5,
    "ev_realized": round((jul18_bet_wins * 0.909 - (jul18_bet_total - jul18_bet_wins)) / max(jul18_bet_total, 1), 4),
...
metrics["today_summary"] = {
    ...
    "sports": ["baseball_mlb"],
    "notes": f"MLB Monday slate (15 games, all series openers). Eval Jul 18: {eval_correct}/{eval_games} correct ({eval_acc:.0%}). Jul 18 bets: {jul18_bet_wins}/{jul18_bet_total} won. OAK 15-1 rout of WSH, COL 10-3 over CIN at Coors."
}
