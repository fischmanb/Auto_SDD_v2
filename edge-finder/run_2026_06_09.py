#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-06-09.
Phase 1: Evaluate 2026-06-06 predictions (last unevaluated).
Phase 2-5: Simulate tonight's games (NHL SCF Game 4 + 15 MLB), produce blended predictions.
"""
...
# ════════════════════════════════════════════════════════════════════════
# PHASE 1 — EVALUATE 2026-06-06 PREDICTIONS
# ════════════════════════════════════════════════════════════════════════
...
# Actual results from June 6, 2026 (fetched via web search)
actual_results = {
    "Carolina Hurricanes @ Vegas Golden Knights": {
        "winner": "home",  # VGK 5, CAR 4 (2OT)
        "home_score": 5,
        "away_score": 4,
        "margin": 1,  # home margin
        "note": "VGK 5-4 2OT, Shea Theodore GWG"
    },
    "Cleveland Guardians @ Texas Rangers": {
        "winner": "away",  # CLE 6, TEX 0
        "home_score": 0,
        "away_score": 6,
        "margin": -6,
        "note": "CLE 6-0, Bibee 8IP 3H 0ER"
    },
    "Boston Red Sox @ New York Yankees": {
        "winner": "postponed",
        "note": "Postponed (rain), makeup Aug 29"
    },
    "Milwaukee Brewers @ Colorado Rockies": {
        "winner": "away",  # MIL 7, COL 1
        "home_score": 1,
        "away_score": 7,
        "margin": -6,
        "note": "MIL 7-1, 5 HR, Misiorowski 103.7mph"
    },
    "Los Angeles Angels @ Los Angeles Dodgers": {
        "winner": "home",  # LAD 9, LAA 2
        "home_score": 9,
        "away_score": 2,
        "margin": 7,
        "note": "LAD 9-2"
    ...
    "New York Mets @ San Diego Padres": {
        "winner": "home",  # SD 3, NYM 2
        "home_score": 3,
        "away_score": 2,
        "margin": 1,
        "note": "SD 3-2, Fermin 1st HR of season"
    },
}
...
for pred in eval_preds["predictions"]:
    ...
    if not actual or actual["winner"] == "postponed":
    ...
    for model in all_models:
        ...
        # Determine which bet type this model would have taken
        evs = {
...
print(f"\n  Games evaluated: {eval_games_played} (1 postponed)")
for m in all_models:
    mid = m["id"]
...
champ_wrong_set = set()
for i, pred in enumerate(eval_preds["predictions"]):
    game_key = pred["game"]
    actual = actual_results.get(game_key)
    if not actual or actual["winner"] == "postponed":
        continue
...
retired = []
for c in challengers:
    if c["weight"] <= 0.15 and c["born"] < "2026-06-04":
        retired.append(c)
...
# ════════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (June 9, 2026)
# ════════════════════════════════════════════════════════════════════════
...
# Sports in season: NHL (Stanley Cup Finals), MLB (regular season)
# NBA: off tonight (Game 4 is Wed June 10)
# NFL: offseason

# RPG formula: 4.5 + (win_pct - 0.5) * 3.0  (same as prior runs)

games_raw = [
    # === NHL Stanley Cup Finals Game 4: Carolina Hurricanes @ Vegas Golden Knights ===
    # Series: VGK leads 2-1. G1: CAR 4-3 2OT, G2: VGK 5-2, G3: VGK 5-4 2OT
    # Carolina: Strong road team, won G1 in Vegas. Svechnikov, Aho, Marner leading scorers.
    # Vegas: Home advantage, momentum from 2 straight wins. Theodore GWG in G3.
    # CAR -115 / VGK -105 on DraftKings. FanDuel: pick'em -110/-110.
    # Puck line: VGK -1.5. O/U: 5.5 (over -130)
    {
        "sport": "icehockey_nhl",
        "home": {
    ...
    # 1. Seattle Mariners (35-32) @ Baltimore Orioles (31-36)
    # SEA -122 / BAL +104. O/U 8.5. Gilbert (4-4, 3.79) vs Rogers (3-6, 6.29)
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 2. Los Angeles Dodgers (42-24) @ Pittsburgh Pirates (34-32)
    # PIT -112 / LAD -104. O/U 8. Skenes (6-5, 3.09) vs Lauer (2-5, 5.74)
    # Skenes at home is dominant. Dodgers sending weak starter.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 3. Boston Red Sox (27-37) @ Tampa Bay Rays (38-25)
    # BOS -110 / TB -106. Tolle vs Martinez.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Tampa Bay Rays",
    ...
    # 4. Minnesota Twins (30-37) @ Detroit Tigers (27-39)
    # DET -126 / MIN +108. Bradley vs Melton.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Detroit Tigers",
    ...
    # 5. Arizona Diamondbacks (34-31) @ Miami Marlins (31-35)
    # MIA -132 / AZ +112. Gallen vs Meyer.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Miami Marlins",
    ...
    # 6. New York Yankees (39-26) @ Cleveland Guardians (37-28)
    # NYY -134 / CLE +114. O/U 8.5. Cole vs TBD.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 7. Oakland Athletics (~27-38) @ Milwaukee Brewers (~38-26)
    # OAK -112 / MIL -104. (A's unexpectedly favored on road — pitching matchup)
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 8. Atlanta Braves (~39-26) @ Chicago White Sox (~22-43)
    # ATL -154 / CWS +130. Big mismatch.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 9. St. Louis Cardinals (~28-37) @ New York Mets (~33-31)
    # NYM -134 / STL +114. O/U 8. May (3-6) vs Peralta (4-4).
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "New York Mets",
    ...
    # 10. Houston Astros (~32-33) @ Los Angeles Angels (~26-39)
    # HOU -120 / LAA +102. O/U 8.5.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 11. Chicago Cubs (34-32) @ Colorado Rockies (24-42)
    # CHC -154 / COL +130. O/U 12.5. Coors Field. Rea (5-3) vs Sugano (5-4).
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 12. Cincinnati Reds (~33-31) @ San Diego Padres (~32-32)
    # CIN -118 / SD +100. O/U 7.5. Burns (7-1) vs Giolito (2-1).
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 13. Philadelphia Phillies (~34-30) @ Toronto Blue Jays (~30-34)
    # PHI -112 / TOR -104.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 14. Washington Nationals (~29-36) @ San Francisco Giants (~28-37)
    # SF -112 / WSH -104.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 15. Texas Rangers (~31-34) @ Kansas City Royals (~28-37)
    # TEX -130 / KC +110. O/U 9.5. Eovaldi (5-6, 4.10) vs Kolek (3-1, 3.32).
    {
        "sport": "baseball_mlb",
        "home": {
...
sport_dates = {"icehockey_nhl": "Tuesday June 9, 2026", "baseball_mlb": "Tuesday June 9, 2026"}
...
# Update rolling stats with June 6 eval
...
# Recalculate rolling 7d and 30d from all_time (approximation)
...
metrics["today_summary"] = {
    ...
    "notes": f"NHL SCF G4 (CAR@VGK) + {sum(1 for p in predictions if p['sport']=='baseball_mlb')} MLB games. {total_bets_today} BETs, {total_leans_today} LEANs, {total_no_today} NO BETs. Eval: Jun 6 was 4/5 correct (BOS@NYY ppd)."
}
...
# ── Summary stats ───────────────────────────────────────────────────
print(f"\n{'=' * 90}")
print(f" EVAL RECAP: Jun 6 predictions — {eval_correct_blend}/{eval_total_blend} blended correct, {bets_won_today}/{len([p for p in bet_preds_eval if actual_results.get(p['game'],{}).get('winner','')!='postponed'])} BETs won")
print(f"{'=' * 90}")
