#!/usr/bin/env python3
"""
Edge-Finder daily runner for 2026-07-10.
...
# Actual results from July 7, 2026 (sourced via web search from ESPN/MLB.com)
actual_results = {
    "Milwaukee Brewers @ St. Louis Cardinals": {"winner": "away", "home_score": 2, "away_score": 10},
    "Chicago Cubs @ Baltimore Orioles": {"winner": "home", "home_score": 3, "away_score": 2},
    "Sacramento Athletics @ Detroit Tigers": {"winner": "home", "home_score": 6, "away_score": 2},
    "Atlanta Braves @ Pittsburgh Pirates": {"winner": "away", "home_score": 5, "away_score": 10},
    "Seattle Mariners @ Miami Marlins": {"winner": "home", "home_score": 6, "away_score": 5},
    "New York Yankees @ Tampa Bay Rays": {"winner": "away", "home_score": 4, "away_score": 12},
    ...
    "Kansas City Royals @ New York Mets": {"winner": "home", "home_score": 7, "away_score": 3},
...
for c in challengers:
    ...
    days_alive = (date(2026, 7, 10) - date(int(born[:4]), int(born[5:7]), int(born[8:10]))).days
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE 2-5 — TONIGHT'S PREDICTIONS (July 10, 2026)
# ═══════════════════════════════════════════════════════════════════════
...
# Friday July 10, 2026 — MLB only (NBA/NHL/NFL off-season)
# 15 games. Odds sourced from FanDuel/DraftKings/BetMGM/Covers/ESPN via web search.
# Records/stats estimated from standings through July 10 and previous run data.

games_raw = [
    # 1. MIL Brewers @ PIT Pirates — 6:40 PM ET
    # MIL 59-34 (best in MLB), PIT ~50-43. Ashcraft vs Sproat.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 2. PHI Phillies @ DET Tigers — 6:40 PM ET
    # PHI 52-42, DET ~49-44. Nola vs Flaherty.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Detroit Tigers",
    ...
    # 3. NYY Yankees @ WAS Nationals — 6:45 PM ET
    # NYY 51-42, WAS ~43-50. Weathers vs Palmquist.
    # Yankees -160 road favorites.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # KC ~42-51, BAL ~47-46. Orioles home favorites.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 5. BOS Red Sox @ NYM Mets — 7:15 PM ET
    # BOS ~46-47, NYM ~36-52. Mets home favorites (pitcher matchup).
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "New York Mets",
    ...
    # 6. SAC Athletics @ CHW White Sox — 7:40 PM ET
    # SAC ~41-47, CHW ~45-42. White Sox home favorites.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 7. LAA Angels @ MIN Twins — 8:10 PM ET
    # LAA ~36-53, MIN ~47-46. Twins home favorites.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 8. SEA Mariners @ TB Rays — 6:50 PM ET
    # SEA ~50-44, TB ~56-37. Very close game, Rays slight home favorites.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Tampa Bay Rays",
    ...
    # 9. ARI Diamondbacks @ LAD Dodgers — 10:10 PM ET
    # ARI ~46-47, LAD ~61-33. Ohtani start. Dodgers heavy favorites.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 10. SF Giants @ COL Rockies — 8:40 PM ET
    # SF ~39-54, COL ~38-57. Both bad, SF favored on pitching (Robbie Ray).
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 11. ATL Braves @ STL Cardinals — 8:15 PM ET
    # ATL ~48-45 (slumping), STL ~44-49. Braves road favorites.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 12. TOR Blue Jays @ SD Padres — 9:40 PM ET
    # TOR ~43-50, SD ~46-47. Near pick'em, slight Padres home edge.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 13. MIA Marlins @ CLE Guardians — 7:10 PM ET
    # MIA surging ~44-49, CLE ~45-48 (no Ramirez). Marlins slight road favorites.
    {
        "sport": "baseball_mlb",
        "home": {
    ...
    # 14. HOU Astros @ TEX Rangers — 8:05 PM ET
    # HOU ~48-45, TEX ~42-51. Quantrill (regression candidate) vs HOU bats.
    {
        "sport": "baseball_mlb",
        "home": {
            "name": "Texas Rangers",
    ...
    # CHC ~48-45, CIN ~46-47. Imanaga pitching for Cubs. High total 9.5.
    {
        "sport": "baseball_mlb",
        "home": {
...
# ── Display results ──────────────────────────────────────────────────
print(f"\n{'=' * 90}")
print(f" MLB — Friday July 10, 2026")
print(f"{'=' * 90}")
...
jul7_bets = [p for p in past_preds["predictions"] if p["verdict"] == "BET"]
jul7_bet_wins = 0
jul7_bet_total = len(jul7_bets)
for bp in jul7_bets:
    gk = bp["game"]
...
new_all_bets = prev_all["total_bets_recommended"] + jul7_bet_total
new_all_bets_won = prev_all["total_bets_won"] + jul7_bet_wins

metrics["last_updated"] = TODAY
metrics["rolling_7d"] = {
    "accuracy": round(eval_correct / eval_games, 4) if eval_games > 0 else 0.5,
    "ev_realized": round((jul7_bet_wins * 0.909 - (jul7_bet_total - jul7_bet_wins)) / max(jul7_bet_total, 1), 4),
...
metrics["today_summary"] = {
    ...
    "sports": ["baseball_mlb"],
    "notes": f"MLB Friday slate (15 games). {total_bets_today} BETs, {total_leans_today} LEANs. Eval Jul 7: {eval_correct}/{eval_games} correct."
}
