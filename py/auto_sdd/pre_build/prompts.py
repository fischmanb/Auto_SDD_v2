...
def personas_user_prompt(project_dir: Path) -> str:
    ...
    return (
        ...
        "These are FUNCTIONAL ARCHETYPES that drive layout and density "
        "decisions, not fictional characters. Do not invent names, ages, "
        "backstories, medical conditions, or personal details. Each "
        "archetype is defined solely by how they use the app.\n\n"
        "Each archetype MUST include only these fields:\n"
        "- **Role** (e.g., 'Power analyst' or 'Executive reviewer')\n"
        "- **Goals** (what they need from the app, 2-3 bullets)\n"
        "- **Device context** (screen size range and typical environment)\n"
        "- **Density preference** (high/medium/low — how much data on "
        "screen at once, and why)\n"
        "- **Critical interactions** (sort, filter, drill-down, etc.)\n"
        "- **Design implication** (one sentence: what this archetype "
        "means for spacing, typography, or hierarchy choices)\n\n"
        "Keep each archetype to 10-15 lines. No prose paragraphs. "
        "The entire file should be under 80 lines.\n\n"
        "Derive archetypes from the vision's target users and the "
...
def design_patterns_user_prompt(project_dir: Path) -> str:
    ...
    return (
        ...
        "- No animation/transition specs unless the vision requires it.\n"
...
def spec_first_user_prompt(
    ...
    return (
        ...
        "in Gherkin steps.\n\n"
        "LENGTH CONSTRAINTS:\n"
        "- S features: 2-4 Gherkin scenarios, 40-80 lines total.\n"
        "- M features: 3-6 Gherkin scenarios, 60-120 lines total.\n"
        "- L features: 4-8 Gherkin scenarios, 80-150 lines total.\n"
        "- User Journey: 3-5 lines max, not a narrative.\n"
        "- Design Token References: table format, not prose.\n"
        "- No component mapping sections. No data flow diagrams. "
        "The build agent gets that from systems-design.md.\n\n"
