...
def design_patterns_user_prompt(project_dir: Path) -> str:
    ...
    return (
        ...
        "CRITICAL: When components are placed side-by-side in the "
        "same grid row, they MUST stretch to equal height (CSS Grid "
        "`align-items: stretch` or Tailwind `items-stretch`). No "
        "dead negative space below the shorter component. If content "
        "heights differ, the shorter component's card extends to "
